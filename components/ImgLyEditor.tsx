...
const ImgLyEditor: React.FC<ImgLyEditorProps> = ({ project }) => {
  ...
  useEffect(() => {
    ...
    if (!licenseKey) {
      addToast("VITE_IMGLY_LICENSE_KEY is not configured.", 'error');
      setIsLoading(false);
      return;
    }
    ...
    const init = async () => {
      ...
      try {
        ...
        // Only touch asset API if editor initialized
        if (editor.asset?.addAssets) {
          await editor.asset.addAssets([
            ...moodboardUrls.map((url, i) => ({ id: `moodboard_${i}`, meta: { uri: url, type: 'image' as const } })),
            ...voiceoverUrls.map((url, i) => ({ id: `voiceover_${i}`, meta: { uri: url, type: 'audio' as const } })),
          ]);
        }
